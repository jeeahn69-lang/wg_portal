...
def get_detail_codes(request, master_cd):
    """
    특정 master_cd에 해당하는 상세 코드 리스트를 반환합니다.
    """
    try:
        ...
        # 데이터가 없을 경우
        if not detail_codes.exists():
            return JsonResponse({"error": "No data found for the given master_cd."}, status=404)

        # JSON 응답 반환
        return JsonResponse(list(detail_codes), safe=False)

    except Exception as e:
        logger.error(f"Error fetching detail codes for master_cd={master_cd}: {str(e)}")
...
@login_required
@csrf_exempt # React 연동 시 필요할 수 있음
@require_http_methods(["DELETE"])
def comcode_master_delete(request, master_cd):
    """공통코드 마스터 삭제"""
    try:
        # 마스터 코드 조회
        master = get_object_or_404(CommonCodeMas, master_cd=master_cd)
        
        # 이름 미리 보관
        master_nm = master.master_nm
        
        # 실제 삭제 (상세코드가 CASCADE 설정이면 상세도 같이 지워집니다)
        master.delete()

        return JsonResponse({
            'success': True,
            'message': f"'{master_nm}' 마스터 코드가 삭제되었습니다."
        })
    ...
    try:
        ...
        if not dtl_idx:
            ...
            try:
                ...
                errors['dtl_idx'] = '상세코드 ID는 정수여야 합니다.'
        ...
        try:
            if sort_ord is None:
                ...
                sort_ord = int(sort_ord)
                if sort_ord < 0:
                    errors['sort_ord'] = '정렬순서는 0 이상이어야 합니다.'
        except (ValueError, TypeError):
            errors['sort_ord'] = '정렬순서는 숫자여야 합니다.'

        # use_yn 검증
        use_yn = use_yn.upper() if use_yn else 'Y'
        if use_yn not in ('Y', 'N'):
            errors['use_yn'] = '사용여부는 Y 또는 N이어야 합니다.'

        if errors:
            return JsonResponse({'success': False, 'errors': errors}, status=422)

        # ── 상세코드 조회 (dtl_idx는 수정 불가) ────────────────────────────────────
        try:
        ...
        # ── DB 수정 (dtl_idx, master_cd, dtl_cd는 수정 불가) ────────────────────────────────────
        detail.dtl_nm = dtl_nm
        detail.sort_ord = sort_ord
        detail.use_yn = use_yn
        detail.save(update_fields=['dtl_nm', 'sort_ord', 'use_yn', 'updated_at'])
