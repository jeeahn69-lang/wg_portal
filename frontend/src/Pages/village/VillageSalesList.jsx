...
export default function VillageSalesList() {
        ...
        <MainLayout>
            ...
            <div className="flex items-center justify-between mb-8">
                <div>
                    ...
                    <Wallet className="w-6 h-6 text-yellow-500" />마을 기업 매출 정보
                    </h1>

                    <p className="text-gray-500 mt-1 font-medium">완주군 내 마을 기업의 매출 정보를 조회합니다.</p>
                </div>
                    ...
                    <table className="w-full text-center border-collapse">
                        <thead>
                            <tr className="bg-gray-200/50">
                                <th className="px-8 py-5 text-sm font-bold text-gray-400 tracking-widest" scope='col'>No.</th>
                                <th className="px-8 py-5 text-sm font-bold text-gray-400 tracking-widest" scope='col'>읍면</th>
                                <th className="px-8 py-5 text-sm font-bold text-gray-400 tracking-widest" scope='col'>마을명</th>
                                <th className="px-8 py-5 text-sm font-bold text-gray-400 tracking-widest" scope='col'>공동체법인명</th>
                                <th className="px-8 py-5 text-sm font-bold text-gray-400 tracking-widest" scope='col'>법인유형</th>
                                <th className="px-8 py-5 text-sm font-bold text-gray-400 tracking-widest" scope='col'>대표자</th>
                                {/* <th className="px-8 py-5 text-sm font-bold text-gray-400 tracking-widest" scope='col'>실무자</th>
                                <th className="px-8 py-5 text-sm font-bold text-gray-400 tracking-widest" scope='col'>마을이장</th> */}
                                <th className="px-8 py-5 text-sm font-bold text-gray-400 tracking-widest" scope='col'>주요제품</th>
                        ...
                        <tbody className="divide-y divide-gray-50">
                            {salesData.map((item, idx) => (
                                <tr key={item.id} className="hover:bg-blue-50/30 transition-colors group cursor-pointer" onClick={() => addTab({ id: `village-info-${item.id}`, title: `${item.village} 정보`, path: '/village/card/', villageName: item.village })}>
                                    ...
                                    <td className="px-8 py-6">
                                        <span className="text-gray-950 font-bold group-hover:text-blue-600 transition-colors text-xm">{item.village}</span>
                                        <div className="text-xs text-gray-400 mt-0.5">{item.type}</div>
                                    </td>
                                    <td className="px-8 py-6 text-xm text-gray-600 font-semibold">{item.corporation}</td>
                                    <td className="px-8 py-6 text-xm text-gray-600 font-medium">{item.cotype}</td>
                                    <td className="px-8 py-6 text-xm text-gray-600 font-medium">{item.ceo}</td>
                                    {/* <td className="px-8 py-6 text-xm text-gray-600 font-medium">{item.worker}</td>
                                    <td className="px-8 py-6 text-xm text-gray-600 font-medium">{item.villhead}</td> */}
                                    <td className="px-8 py-6">
