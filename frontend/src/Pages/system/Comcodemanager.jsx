...
    return (
        <MainLayout>
            <div className="font-sans min-h-0 flex flex-col bg-gray-50/50 mb-8">
                {/* 상단 헤더 영역 - 고정 */}
                <div className="mb-6 flex justify-between items-end flex-none">
                    <div>
...
                            엑셀 내보내기
                        </button>
                        <button className="px-5 py-2.5 bg-blue-600 text-white rounded-xl shadow-lg shadow-blue-100 hover:bg-blue-700 font-bold text-xs transition-all active:scale-95">
                            신규 대분류 등록
                        </button>
                    </div>
                </div>
...
                {/* 메인 그리드 영역 - 화면 높이에 맞춤 */}
                 <div className="grid grid-cols-12 gap-6 flex-none h-[480px] ">
                    {/* 왼쪽 영역: 검색 + 대분류 */}
                    <div className="col-span-12 lg:col-span-5 flex flex-col gap-6 h-full min-h-0">
                        {/* 검색 카드 */}
                        <div className="bg-white rounded-[24px] shadow-sm border border-gray-100 p-5 flex-none border-blue-100 shadow-xl transition-all">
...
                                        <input 
                                            type="text" 
                                            className="px-4 py-2 bg-gray-50 border-none rounded-xl text-sm focus:ring-2 focus:ring-cyan-100 transition-all"
...
                        {/* 대분류 리스트 - 스크롤 적용 */}
                        <div className="bg-white rounded-[32px] shadow-sm border border-gray-100 overflow-hidden flex flex-col flex-none h-[474px] overflow-hidden border-blue-100 shadow-xl transition-all">
                            <div className="p-6 border-b border-gray-50 bg-gray-50/30 flex-none">
...
                                                className={`cursor-pointer transition-all border-b border-gray-50 ${selectedMaster === item.master_cd ? 'bg-blue-50/80 font-bold border-l-4 border-l-blue-600' : 'hover:bg-gray-50'}`}
                                            >
                                                <td className="px-8 py-4 text-gray-900">{item.master_cd}</td>
                                                <td className="px-8 py-4">{item.master_nm}</td>
                                                <td className="px-8 py-4 text-center">
                                                    <span className={`px-2 py-1 rounded-md text-[10px] font-black ${item.use_yn === 'Y' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'}`}>
...
                    {/* 오른쪽 영역: 소분류 상세 - 스크롤 적용 */}
                    <div className="col-span-12 lg:col-span-7 bg-white rounded-[32px] shadow-sm border border-gray-100 overflow-hidden flex flex-col flex-none h-[600px] overflow-hidden border-blue-100 shadow-xl transition-all">
                        <div className="p-6 border-b border-gray-50 flex justify-between items-center bg-gray-50/30 flex-none">
                            <h3 className="font-black text-gray-800 flex items-center gap-2 text-lg">
...
                            <button className="px-3 py-1.5 text-[13px] font-black bg-indigo-50 text-indigo-600 rounded-lg hover:bg-indigo-100 transition-colors">소분류 추가 +</button>
                        </div>
                        <div className="overflow-y-auto flex-none">
                            {selectedMaster ? (
                                isLoading ? (
...
                                        <thead className="bg-gray-50/80 sticky top-0 z-10 backdrop-blur-sm">
                                            <tr className="text-gray-400 text-sm uppercase font-black tracking-widest">
                                                <th className="px-8 py-4">소분류코드</th>
                                                <th className="px-8 py-4">코드명</th>
